...
# If you have plenty of time and a fast computer, then run a full
# model, discover that not all terms are significant, do a model selection
# and end up with a model that contains the covariates:
#   Species + PreySizeClass + ColonySize.c +
#   PreySizeClass : ColonySize.c + (1 | Colony)

# But given the fact that is is towards the end of the week, just fit 
# the optimal model above. This was M2 from the frequentist approach.
...
#First we need to remove rows for which trials = 0
Spiders2 <- Spiders[Spiders$Respondents > 0,]
dim(Spiders)
dim(Spiders2) #We should have picked this up in the frequentist analysis
...
win.data  <- list(Y      = Spiders2$Attackers, #Response  
...
p <- K  + 1
sum(Ep^2) / (N - p)
#Tja...there is overdispersion...just a little bit!
...
#3. Calculate for each MCMC iteration the predicted values on this grid.
