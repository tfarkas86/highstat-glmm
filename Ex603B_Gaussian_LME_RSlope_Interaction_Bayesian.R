...
#  -Petiole_Diameter and Beaver absence/presence.
...
#JAGS doesn't like factors as character strings. It wants to
#have them as 0-1 variables.
LS$Beaver01 <- as.numeric(LS$Beaver) - 1

# This is the model:
# MD_ij ~ N(mu_ij, sigma^2)
# E(MD_ij) = beta_1 + beta_2 * PD.std  + 
#            beta_3 * Beaver01 + beta_4 * PD.std + #Fixed stuff 
...
win.data <- list(MD     = LS$MD,         #Response
                 PD.std = LS$PD.std,     #Covariate
                 Beaver01 = LS$Beaver01, #Covariate
...
model{
    ...
    for (i in 1:356) {
      MD[i]   ~ dnorm(mu[i], tau)
      eta[i] <- beta[1] + beta[2] * PD.std[i]  + 
                beta[3] * Beaver01[i] + 
                beta[4] * Beaver01[i] * PD.std[i]  #Covariates
...
MyNames <- c("Intercept", "PD", "Beaver", "Interaction PD beaver",
             "sigma eps", "sigma1 site", "sigma2 site")
#Adjust this variable if extra parameters (e.g an ICC) are added!


MyBUGSChains(outG, 
             c(uNames("beta", 4), "sigma", "sigma1_Site", "sigma2_Site"),
             PanelNames = MyNames)
...
#TASK: Write the equation for the model



#Sketch the fitted values with CI
range(LS$PD.std)   #-2.337537  2.479650
MyData <- expand.grid(PD.std = seq(-2.34, 2.48, length = 25),
                      Beaver01 = c(0,1 ))
Xp     <- model.matrix(~ PD.std * Beaver01, data = MyData)
dim(Xp)
...
p <- p + geom_ribbon(data = MyData2, 
                     aes(x = PD, 
                         ...
                         ymin = lo ),
                     alpha = 0.4)
p <- p + facet_grid(. ~ Beaver01, scales = "fixed")
                     
p


#We are extrapolating!!
#Use the MyData in the code above:

library(plyr)
MyData <- ddply(LS, .(Beaver01), summarize,
