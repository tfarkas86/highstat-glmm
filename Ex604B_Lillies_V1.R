...
#NEW TASK: USE GENERAL CODING so that we don't have to
#          change anything for a new data set
...
X <- model.matrix(~ PD.std + Beaver, data = LS)
...
#            beta_3 * Beaver01  +       # X * beta Fixed stuff 
#            a_i                        # Random stuff
...
# and site index
win.data <- list(Y    = LS$MD, #Response
                 X    = X,     #Covariate
                 K    = K,     #Number of betas
                 N    = nrow(LS), #Sample size
...
Xp     <- model.matrix(~ PD.std + Beaver, data = MyData)
dim(Xp)
